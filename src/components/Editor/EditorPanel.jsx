                ...
                <div style="
                  ...
                  " onclick="openMultipleCardModal${elementId.replace(/-/g, '_')}(${index})" data-card-index="${index}" 
                    ...
                    ${(() => {
                        ...
                        console.log('🔥🔥🔥 FILENAME-DEBUG: EditorPanel - Processing card ' + card.id);
                        console.log('🔥🔥🔥 FILENAME-DEBUG: EditorPanel - card.fileName = "' + (card.fileName || '') + '"');
                        console.log('🔥🔥🔥 FILENAME-DEBUG: EditorPanel - card.imageUrl = "' + (card.imageUrl || '') + '"');
                        
                        // ПРИОРИТЕТ 1: Используем card.fileName (самый надежный источник)
                        if (card.fileName && card.fileName.startsWith('card_')) {
                          cardImageSrc = 'assets/images/' + card.fileName;
                          console.log('🔥🔥🔥 FILENAME-DEBUG: EditorPanel - Using fileName: ' + cardImageSrc);
                        }
                        // ПРИОРИТЕТ 2: Проверяем глобальный маппинг только если нет fileName
                        else if (window.cardImageFileMap && card.id) {
                          const exportedFileName = window.cardImageFileMap.get(card.id);

                          console.log('🔥🔥🔥 FILENAME-DEBUG: EditorPanel - cardImageFileMap result: "' + (exportedFileName || '') + '"');
                          if (exportedFileName) {

                            cardImageSrc = 'assets/images/' + exportedFileName;

                            console.log('🔥🔥🔥 FILENAME-DEBUG: EditorPanel - Using exported file: ' + cardImageSrc + ' (cardId: ' + card.id + ')');
                          } else {

                            console.log('🔥🔥🔥 FILENAME-DEBUG: EditorPanel - No mapping found for card ID: ' + card.id);
                          }
                        }
                        // ПРИОРИТЕТ 3: Fallback к imageUrl
                        else if (card.imageUrl && card.imageUrl.trim() && !card.imageUrl.includes('placeholder')) {
                            cardImageSrc = card.imageUrl;

                          console.log('🔥🔥🔥 FILENAME-DEBUG: EditorPanel - Using imageUrl: ' + cardImageSrc);
                        }
                        
                        console.log('🔥🔥🔥 FILENAME-DEBUG: EditorPanel - FINAL SRC: ' + cardImageSrc);
                        
                        // Показываем изображение только если оно реально есть
                        if (cardImageSrc && cardImageSrc.trim() && !cardImageSrc.includes('placeholder')) {
                          return `
                            <div class="service-image" style="
                              width: 100%;
                              height: 140px;
                              ...
                              flex-shrink: 0;
                            ">
                              <img src="${cardImageSrc}" alt="${card.imageAlt || card.title || 'Изображение'}" style="
                          width: 100%;
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  // 🆕 Генерация классической шапки (полная функциональность)
  const generateClassicHeaderJS = (siteData) => {
    const headerData = siteData.headerData || {};
    const siteName = headerData.siteName || 'My Site';
    const sectionsArray = Object.entries(siteData.sectionsData || {});
    ...
    return `// 🎯 Классическая шапка - полная функциональность
// Включает: логотип, полное меню, мобильную версию, стили
...
`;
  ...
  // 🆕 Генерация минималистичной шапки в стиле Groovy Gang
  const generateMinimalHeaderJS = (siteData) => {
    const headerData = siteData.headerData || {};
    const siteName = headerData.siteName || 'My Site';
    const sectionsArray = Object.entries(siteData.sectionsData || {});
  ...
  function injectHeaderStyles() {
    ...
    style.textContent = \`
      ...
      .sidebar-menu {
        ...
        height: 100vh;
        width: 320px;
        background: linear-gradient(135deg, ${headerData.backgroundColor || '#1a1a1a'}, ${headerData.backgroundColor ? darkenColor(headerData.backgroundColor, 20) : '#2d2d2d'});
        ...
        border-right: 1px solid ${headerData.linksColor ? headerData.linksColor + '20' : 'rgba(255, 107, 53, 0.2)'};
      }
      ...
      .sidebar-menu::before {
        ...
        right: 0;
        bottom: 0;
        background: linear-gradient(45deg, transparent 0%, ${headerData.linksColor ? headerData.linksColor + '05' : 'rgba(255, 107, 53, 0.05)'} 50%, transparent 100%);
        opacity: 0;
      ...
      .sidebar-header {
        ...
        margin-top: 2rem;
        padding-bottom: 1rem;
        border-bottom: 2px solid ${headerData.linksColor ? headerData.linksColor + '40' : 'rgba(255, 107, 53, 0.3)'};
      }
      ...
      .sidebar-title {
        ...
        font-size: 1.3rem;
        font-weight: bold;
        text-shadow: 0 0 15px ${headerData.linksColor ? headerData.linksColor + '60' : 'rgba(255, 107, 53, 0.4)'};
        margin: 0;
      }
      ...
      .sidebar-close:hover {
        background: ${headerData.linksColor ? headerData.linksColor + '20' : 'rgba(255, 107, 53, 0.2)'};
        color: ${headerData.linksColor || '#ff6b35'};
        transform: scale(1.1);
      }
...
`;
  ...
  const generateHeaderJS = (siteData) => {
    ...
    switch(format) {
      case 'classic':
        return generateClassicHeaderJS(siteData);
      case 'minimal':
        return generateMinimalHeaderJS(siteData);
      default:
        return generateClassicHeaderJS(siteData);
    }
  };
...
<body>
  ...
  const generateMultiPageSection = (siteData, sectionId, sectionData) => {
    ...
    // 🔥 ИСПРАВЛЕНИЕ: НЕ применяем sectionBackground из элементов к секции
    // sectionBackground должен применяться только к отдельным элементам, а не к секции целиком
    // Комментируем этот код, чтобы избежать применения фона элементов ко всей секции
    /*
    if (!sectionColorSettings.sectionBackground?.enabled) {
          ...
          try {
            ...
            if (metadata && (
              ...
              if (imageBlob) {
                ...
                // Используем оригинальное имя файла вместо переименования
                const cleanFileName = metadata.fileName;
                imagesFolder.file(cleanFileName, imageBlob);

                foundCardImages++;

                console.log(`🔥EXPORT🔥 Added card image from regular cache: ${cleanFileName} (using original name)`);
              }
                ...
                try {
                  ...
                  if (element.fileName) {
                    ...
                    if (imageBlob) {
                      ...
                      const cleanFileName = element.fileName;
                      imagesFolder.file(cleanFileName, imageBlob);

                      directCardImages++;

                      console.log(`🔥EXPORT🔥 Added card image by fileName: ${cleanFileName} (using original name)`);
                    }
